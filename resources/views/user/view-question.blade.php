@extends('layouts.app')

@section('content')
<div class="container py-5 px-5">
    <div class="w-85 mx-auto">
        {{-- question zone --}}
        <h1 class="dark-purple">How can I join Kredo Soccer Club?</h1>
        <div class="category-label w-15 ms-auto text-center mt-1 mb-3">
            <p>Club</p>
        </div>
        {{-- user icon and name --}}
        <div class="row">
            <div class="col-1 text-center">
                <a href="{{route('profile')}}">
                    <i class="fa-solid fa-circle-user icon-sm text-secondary"></i>
                </a>
            </div>
            <div class="col ms-auto d-flex align-items-center">
                <a href="{{route('profile')}}" class="text-decoration-none">
                    <p class="fs-4 m-0 thick-gray">Yukari</p>
                </a>
            </div>
            <div class="col text-end my-auto">
                <button class="btn btn-none purple-gray" type="button" data-bs-toggle="modal" data-bs-target="#report-question-"><i class="fa-regular fa-flag"></i> Report this post</button>
            </div>
        </div>
        <div class="mt-4 px-2">
            <p class="fs-5">
                I wanna join the soccer club, but I don't know where I should go to join there. Also, is there any fee or requirements for joining that?
            </p>
        </div>
        {{-- image space --}}
        <div class="mt-2">
            <img src="" alt="">
        </div>

        {{-- p.s. posted --}}
        <div class="container w-75 mt-2">
            <h2 class="second-title">P.S.</h2>
            <div class="my-4 px-2">
                <p class="fs-5">
                    I wanna join the soccer club, but I don't know where I should go to join there. Also, is there any fee or requirements for joining that?
                </p>
            </div>
        </div>

        {{-- heart and time --}}
        <div class="mt-2 text-end ">
            <form action="" class="d-inline">
                @csrf
                <button type="submit" class="btn btn-none px-0">
                    <i class="fa-regular fa-heart purple-gray"></i>
                </button>
                <span class="purple-gray">1</span>
                <span class="ms-3 purple-gray">2024/3/26 15:05</span>
            </form>
            <button type="button" class="trash-btn ms-2" data-bs-toggle="modal" data-bs-target="#delete-question-">
                <i class="fa-solid fa-trash red"></i>
            </button>
        </div>

        {{-- write p.s. --}}
        {{-- <form action="">
            @csrf
            <div class="container w-75 mt-2">
                <h2 class="second-title">Add additional Info</h2>
                <textarea name="" id="" rows="10" class="w-100 big-textarea px-2 py-2" placeholder=" Write your postscript in here!"></textarea>
                <button type="button" class="post-btn w-100 py-1 mt-2" data-bs-toggle="modal" data-bs-target="#post-ps-">Post</button>
            </div>

            <div class="modal fade" id="post-ps-" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header w-100 mx-auto ">
                            <h3 class="modal-title dark-purple" id="exampleModalLongTitle">Post P.S.</h3>
                        </div>
                        <form action="">
                            @csrf
                            <div class="modal-body text-start">
                                <p class="dark-purple">Are you sure you want to post this p.s.?</p>
                            </div>
                            <div class="modal-footer pb-3 border-0 pt-3">
                                <div class="w-100 mx-auto row">
                                    <div class="col text-start">
                                        <button type="button" class="cancel py-1 w-50" data-bs-dismiss="modal">Cancel</button>
                                    </div>
                                    <div class="col text-end">
                                        <button type="submit" class="execute w-50 py-1">Post</button>
                                    </div>
                                </div>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </form> --}}

    </div>

    <hr class="mt-3 w-85 mx-auto">

    {{-- answer area --}}
    <div class="w-75 mx-auto mt-5" id="answer-1">
        <h2 class="second-title">Answers</h2>
        <div class="row mt-5">
            <div class="col-1 text-center">
                <a href="{{route('profile')}}">
                    <i class="fa-solid fa-circle-user icon-sm text-secondary"></i>
                </a>
            </div>
            <div class="col ms-auto d-flex align-items-center">
                <a href="{{route('profile')}}" class="text-decoration-none">
                    <p class="fs-4 m-0 thick-gray">Kredo Taro</p>
                </a>
            </div>
            <div class="col text-end">
                <button class="btn btn-none purple-gray" type="button" data-bs-toggle="modal" data-bs-target="#report-answer-"><i class="fa-regular fa-flag"></i> Report this post</button>
            </div>
        </div>
        <div class="mt-4 px-2">
            <p class="fs-5">
                I wanna join the soccer club, but I don't know where I should go to join there. Also, is there any fee or requirements for joining that?
            </p>
        </div>
        <div class="mt-2 text-end ">
            <form action="" class="d-inline">
                @csrf
                <button type="submit" class="btn btn-none px-0">
                    <i class="fa-regular fa-heart purple-gray"></i>
                </button>
                <span class="purple-gray">1</span>
                <span class="ms-3 purple-gray">2024/3/26 15:05</span>
            </form>
            {{-- <button type="button" class="trash-btn ms-2" data-bs-toggle="modal" data-bs-target="#delete-answer-">
                <i class="fa-solid fa-trash red"></i>
            </button> --}}
        </div>
    </div>

    <hr class="mt-3 w-75 mx-auto">

    <div class="w-75 mx-auto mt-5" id="answer-2">
        <div class="row mt-5">
            <div class="col-1 text-center">
                <a href="{{route('profile')}}">
                    <i class="fa-solid fa-circle-user icon-sm text-secondary"></i>
                </a>
            </div>
            <div class="col ms-auto d-flex align-items-center">
                <a href="{{route('profile')}}" class="text-decoration-none">
                    <p class="fs-4 m-0 thick-gray">Kredo Taro</p>
                </a>
            </div>
            <div class="col text-end">
                <button class="btn btn-none purple-gray" type="button" data-bs-toggle="modal" data-bs-target="#report-answer-"><i class="fa-regular fa-flag"></i> Report this post</button>
            </div>
        </div>
        <div class="mt-4 px-2">
            <p class="fs-5">
                I wanna join the soccer club, but I don't know where I should go to join there. Also, is there any fee or requirements for joining that?
            </p>
        </div>
        <div class="mt-2 text-end ">
            <form action="" class="d-inline">
                @csrf
                <button type="submit" class="btn btn-none px-0">
                    <i class="fa-regular fa-heart purple-gray"></i>
                </button>
                <span class="purple-gray">1</span>
                <span class="ms-3 purple-gray">2024/3/26 15:05</span>
            </form>
            {{-- <button type="button" class="trash-btn ms-2" data-bs-toggle="modal" data-bs-target="#delete-answer-">
                <i class="fa-solid fa-trash red"></i>
            </button> --}}
        </div>
    </div>


    {{-- write an answer --}}

    {{-- <hr class="mt-3 w-85 mx-auto">
    <form action="">
        <div class="container w-75 mt-2">
            <h2 class="second-title">Write your answer</h2>
            <div class="mt-4">
                <textarea name="" id="" rows="10" class="w-100 big-textarea px-2 py-2" placeholder=" Write your answer in here!"></textarea>
                <input type="file" class="form-control mt-3">
                <label for="" class="form-text purple-gray">Accepted file types: jpg, jpeg, png, gif, Max file size 1048kb.</label>
                <button type="button" class="post-btn w-100 py-1 mt-3" data-bs-toggle="modal" data-bs-target="#post-answer-">Post</button>
            </div>
        </div>

        <div class="modal fade" id="post-answer-" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header w-100 mx-auto ">
                        <h3 class="modal-title dark-purple" id="exampleModalLongTitle">Post Answer</h3>
                    </div>
                    <form action="">
                        @csrf
                        <div class="modal-body text-start">
                            <p class="dark-purple">Are you sure you want to post this answer?</p>
                        </div>
                        <div class="modal-footer pb-3 border-0 pt-3">
                            <div class="w-100 mx-auto row">
                                <div class="col text-start">
                                    <button type="button" class="cancel py-1 w-50" data-bs-dismiss="modal">Cancel</button>
                                </div>
                                <div class="col text-end">
                                    <button type="submit" class="execute w-50 py-1">Post</button>
                                </div>
                            </div>
                        </div>
                    </form>
                </div>
            </div>
        </div>

    </form> --}}


    {{-- report question popup --}}
    <div class="modal fade" id="report-question-" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header border-0 w-80 mx-auto pb-0 pt-3">
                    <h3 class="modal-title dark-purple" id="exampleModalLongTitle">Report Question</h3>
                </div>
                <div class="modal-body">
                    <form action="" method="POST">
                        @csrf
                        <div class="text-center">
                            <select class="create-q-select px-1 mb-3">
                                <option selected>Category of Problem</option>
                                <option value="1">One</option>
                                <option value="2">Two</option>
                                <option value="3">Three</option>
                            </select>
                            <textarea name="" id="" rows="10" class="w-80 big-textarea px-3 py-2" placeholder="Please write the reason of reporting this team as detailed as possible."></textarea>
                        </div>
                    </form>
                </div>
                <div class="modal-footer border-0 pb-3">
                    <div class="w-80 mx-auto row">
                        <div class="col text-end">
                            <button type="button" class="create-q-cancel py-1 w-100" data-bs-dismiss="modal">Close</button>
                        </div>
                        <div class="col">
                            <button type="submit" class="create-q-post-btn w-100 py-1">Post</button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    {{-- report answer popup --}}
    <div class="modal fade" id="report-answer-" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header border-0 w-80 mx-auto pb-0 pt-3">
                    <h3 class="modal-title dark-purple" id="exampleModalLongTitle">Report Answer</h3>
                </div>
                <div class="modal-body">
                    <form action="" method="POST">
                        @csrf
                        <div class="text-center">
                            <select class="create-q-select px-1 mb-3">
                                <option selected>Category of Problem</option>
                                <option value="1">One</option>
                                <option value="2">Two</option>
                                <option value="3">Three</option>
                            </select>
                            <textarea name="" id="" rows="10" class="w-80 big-textarea px-3 py-2" placeholder="Please write the reason of reporting this team as detailed as possible."></textarea>
                        </div>
                    </form>
                </div>
                <div class="modal-footer border-0 pb-3">
                    <div class="w-80 mx-auto row">
                        <div class="col text-end">
                            <button type="button" class="create-q-cancel py-1 w-100" data-bs-dismiss="modal">Close</button>
                        </div>
                        <div class="col">
                            <button type="submit" class="create-q-post-btn w-100 py-1">Post</button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    {{-- delete question modal --}}
    <div class="modal fade" id="delete-question-" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header w-100 mx-auto ">
                    <h3 class="modal-title red" id="exampleModalLongTitle">Delete Question</h3>
                </div>
                <form action="">
                    @csrf
                    <div class="modal-body text-start">
                        <p class="red">Are you sure you want to delete this question?</p>
                    </div>
                    <div class="modal-footer pb-3 border-0 pt-3">
                        <div class="w-100 mx-auto row">
                            <div class="col text-start">
                                <button type="button" class="delete-team-cancel py-1 w-50" data-bs-dismiss="modal">Cancel</button>
                            </div>
                            <div class="col text-end">
                                <button type="submit" class="delete-team-post-btn w-50 py-1">Delete</button>
                            </div>
                        </div>
                    </div>
                </form>
            </div>
        </div>
    </div>

    {{-- delete answer popup --}}
    <div class="modal fade" id="delete-answer-" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header w-100 mx-auto ">
                    <h3 class="modal-title red" id="exampleModalLongTitle">Delete Answer</h3>
                </div>
                <form action="">
                    @csrf
                    <div class="modal-body text-start">
                        <p class="red">Are you sure you want to delete this answer?</p>
                    </div>
                    <div class="modal-footer pb-3 border-0 pt-3">
                        <div class="w-100 mx-auto row">
                            <div class="col text-start">
                                <button type="button" class="delete-team-cancel py-1 w-50" data-bs-dismiss="modal">Cancel</button>
                            </div>
                            <div class="col text-end">
                                <button type="submit" class="delete-team-post-btn w-50 py-1">Delete</button>
                            </div>
                        </div>
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>
@endsection
